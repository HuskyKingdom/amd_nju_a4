...
class ParallelVocabEmbedding(nn.Module):
    ...
    def __init__(self, 
        ...
        self.r = rank
        self.w = world_size
        self.n = self.v//self.w
        self.init_mean = init_mean
        self.init_std = init_std
        self.init_base_seed = init_base_seed
        self.dtype = dtype
        self.device = device
        self.vocab_start_index = self.r * self.n
        self.vocab_end_index = (self.r + 1) * self.n - 1
    ...
    def forward(self, input_ids: torch.LongTensor) -> torch.Tensor:
        ...
        maksed_input = input_ids.clone() - self.vocab_start_index
        maksed_input[input_mask] = 0
        E = F.embedding(maksed_input,self.Tr.to(maksed_input.device))
        E[input_mask,:] = 0
        return E.to(self.dtype).to(input_ids.device)
        
        
    def reset_parameters(self) -> None:
        """Initialize learnable embedding parameters for Vocab Embedding from a normal distribution"""
        torch.manual_seed(self.init_base_seed + self.r)
        self.Tr = nn.Parameter(torch.empty((self.n,self.e),device=self.device,dtype=self.dtype).normal_(mean=self.init_mean,std=self.init_std),requires_grad=True)
