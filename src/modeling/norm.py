...
class GroupRMSNorm(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.h = hidden_size
        self.gz = self.h if group_size is None else group_size
        self.epsilon = eps
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        b,s,h = input.shape
        input = input.view(b,s,-1,self.gz).to(torch.float32)
        variance = input.pow(2).mean(dim=-1,keepdim=True)
        output = (self.weight.view(-1,self.gz).to(input_device) * (input * torch.rsqrt(variance + self.epsilon))).view(b,s,h)
    ...
    def reset_parameters(self) -> None:
        ...
        # set uniform tensor
        self.weight = nn.Parameter(torch.empty(self.h, device=self.device, dtype=self.dtype).uniform_(self.init_range[0], self.init_range[1]),requires_grad=True)
