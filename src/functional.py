...
def apply_rotary_pos_emb(
    ...
    input = input.to(torch.float32).transpose(1,2)
    cos = cos.to(input.device)
    sin = sin.to(input.device)
    input_embed = ((input * cos) + (rotate_half(input) * sin)).transpose(1,2)
    return input_embed.to(input_dtype).to(input_device)
